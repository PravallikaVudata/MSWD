...
const App=()=>{
  ...
  const CountryList=({country,countries,setCountry})=>{
    const Res=countries.filter(count=>count.name.toLowerCase().includes(country.toLowerCase()));
    return(
      <div>
        {
          Res.length > 10?
          ...
          :
          Res.length ===1?
          <DetailsofCountry country={Res[0]}/>
          :
          <ul style={{listStyle: 'none', padding: 0}}>
            {
              Res.map(Res=>(
                <l1 key={Res.name}>
                  {Res.name}<button onClick={()=>setCountry(Res.name)}>show</button><br></br>
                </l1>
              ))
            }
          </ul>
